...
class DataLoaderEMNIST:
    def __init__(self, train_batch_size, test_batch_size, shuffle=True):
        self.train_dataloader = DataLoader(
            datasets.EMNIST(root="data", split="byclass", train=True, download=True, transform=ToTensor()),
            batch_size=train_batch_size, shuffle=shuffle)

        self.test_dataloader = DataLoader(
            datasets.EMNIST(root="data", split="byclass", train=False, download=True, transform=ToTensor()),
...
class TrainingManager:
    ...
    def train_epoch(self):
        ...
        self.history['train_accuracy'].append(100.0 * train_correct / len(self.train_loader.dataset))
    ...
    def fit(self, epochs):
        for epoch in range(epochs):
            ...
            self.train_epoch()
            self.test_epoch()
            print(f'Train Loss: {self.history["train_loss"][-1]:.4f}, Train Acc: {self.history["train_accuracy"][-1]:.2f}%')
            print(f'Test Loss: {self.history["test_loss"][-1]:.4f}, Test Acc: {self.history["test_accuracy"][-1]:.2f}%')
...
if __name__ == "__main__":
    ...
    learning_rate = 1e-2
    epochs = 10
    dataloader = DataLoaderEMNIST(batch_size, 2056)
    ...
    # Training Manager
    manager = TrainingManager(model, dataloader, optimizer, loss_fn, device, scheduler=None)
    manager.fit(epochs)
